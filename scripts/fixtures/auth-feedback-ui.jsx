...
function TwoFactorSetupHarness() {
  ...
  const state = useSettingsMyAccountTwoFactorState({ currentUser, isMountedRef: useRef(true), syncCurrentUser: setCurrentUser, toast: (value) => setNotice(value.description) });
  const ignore = () => {};
  return <main style={{ maxWidth: 960, margin: "24px auto", padding: 16 }}>
    ...
    <MyAccountSecurityCard
      ...
      onChangePassword={ignore} onChangeUsername={ignore} onConfirmPasswordBlur={ignore} onConfirmPasswordInputChange={ignore}
      onCurrentPasswordBlur={ignore} onCurrentPasswordInputChange={ignore} onNewPasswordBlur={ignore} onNewPasswordInputChange={ignore}
      onUsernameBlur={ignore} onUsernameInputChange={ignore} passwordSaving={false}
      twoFactorEnabled={currentUser.twoFactorEnabled} twoFactorPendingSetup={currentUser.twoFactorPendingSetup}
      usernameError={null} usernameInput="ui.fixture" usernameSaving={false}
    />
  </main>;
}
