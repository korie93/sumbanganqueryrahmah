...
test("secret rotation runbook gates deployment-server rebuild validation", async () => {
  ...
  const firstDeployRestartIndex = content.indexOf("Deploy or restart", gateIndex);
  ...
  assert.ok(firstDeployRestartIndex > gateIndex, "secret rotation restart steps should follow checkout verification");
  assert.ok(buildIndex > gateIndex, "secret rotation validation build should follow checkout verification");
  assert.match(content, /Do not deploy, rebuild, restart workers, or continue a production rotation/);
});
