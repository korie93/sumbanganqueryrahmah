...
const pgBin = process.env.SQR_COLLECTION_CARD_TEST_PG_BIN
...
async function waitUntilReady(baseUrl, child, failedToStart) {
  ...
  while (Date.now() < deadline) {
    if (failedToStart()) throw new Error("Disposable Collection Card server process or its log could not initialize.");
    if (child.exitCode !== null || child.signalCode !== null) throw new Error("Disposable Collection Card app exited before health readiness. Inspect its sanitized server log.");
    try {
  ...
  throw new Error("Disposable Collection Card app did not become healthy within 120 seconds.");
}
...
async function main() {
  assert.ok(process.argv.slice(2).every((arg) => arg === "--expect-missing-card"), "Unknown fixture argument");
  const expectMissingCard = process.argv.includes("--expect-missing-card");
  ...
  const { runCollectionCardNoBrowser } = await import("./collection-card-no-browser.mjs");
  ...
  const artifactParent = path.join(repoRoot, "artifacts", "collection-card-no");
  ...
  try {
    ...
    const limitedAccounts = ["admin", "user"].map((role) => ({
      role, username: `${role}cardfixture${randomBytes(6).toString("hex")}`,
      password: `Fixture9!${randomBytes(24).toString("base64url")}`,
    }));
    ...
    const admin = new pg.Client({ host: "127.0.0.1", port: databasePort, user: "sqr_fixture", password: fixtureDatabasePassword, database: "postgres", ssl: false, application_name: "sqr-collection-card-no-disposable", connectionTimeoutMillis: 3_000 });
    try {
      await admin.connect();
    ...
    const env = {
      ...
      PG_HOST: "127.0.0.1", PG_PORT: String(databasePort), PG_USER: "sqr_fixture", PG_PASSWORD: fixtureDatabasePassword, PG_DATABASE: "sqr_collection_card_test", DATABASE_SSL: "0",
      ...
      SEED_DEFAULT_USERS: "1", SEED_SUPERUSER_USERNAME: username, SEED_SUPERUSER_PASSWORD: password, SEED_SUPERUSER_FULL_NAME: "Disposable Collection Card Test",
      SEED_ADMIN_USERNAME: limitedAccounts[0].username, SEED_ADMIN_PASSWORD: limitedAccounts[0].password,
      SEED_USER_USERNAME: limitedAccounts[1].username, SEED_USER_PASSWORD: limitedAccounts[1].password,
    ...
    if (spawnFailure) throw new Error("Disposable Collection Card server process could not start.");
    await run(process.execPath, ["--import", pathToFileURL(path.join(repoRoot, "node_modules/tsx/dist/loader.mjs")).href,
      ...
      ...env, SQR_COLLECTION_CARD_ISOLATED_CLUSTER: "1", SQR_COLLECTION_CARD_DATA_DIR: dataDir,
      COLLECTION_CARD_EXPECT_MISSING: expectMissingCard ? "1" : "0",
    });
    console.log("Disposable built app and PostgreSQL ready; real Collection Card browser verification starting.");
    await runCollectionCardNoBrowser({ baseUrl, username, password, limitedAccounts, artifactsDir, expectMissingCard });
    console.log(`${expectMissingCard ? "EXPECTED BASELINE FAILURE REPRODUCED" : "Collection Card verification passed"}. Synthetic screenshots and sanitized server log: ${artifactsDir}`);
  } catch (error) {
    failed = true;
    // Browser tests must not print their entered credentials, secret or OTP.
    console.error(error?.code === "ERR_ASSERTION" ? error.message : "Isolated Collection Card verification failed; inspect the last safe phase marker and redacted artifacts. Raw browser errors are suppressed because they can include entered secrets.");
    console.error(`Collection Card verification artifacts: ${artifactsDir}`);
  } finally {
    ...
    if (appStopped && databaseStopped) {
      try {
        ...
        console.log("Disposable Collection Card app/database removed. Existing application uploads, receipts and data were not used.");
      } catch {
        failed = true;
...
main().catch(() => {
  console.error("Isolated Collection Card verification could not initialize. Build the app and check the browser module and local PostgreSQL binaries.");
  process.exitCode = 1;
});
