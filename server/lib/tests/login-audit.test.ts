...
test("buildLoginFailureAuditDetails records useful masked context without secrets", () => {
  ...
  assert.equal(details.includes("203.0.113.42"), false);
  ...
  assert.equal(verified.entry.metadata.network, "203.0.x.x");
