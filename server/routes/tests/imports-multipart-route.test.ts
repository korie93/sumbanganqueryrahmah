...
test("createImportsMultipartRoute cleans up when the request is aborted mid-upload", async () => {
  const quotaTracker = createActiveImportUploadQuotaTracker(1024);
  const handler = createImportsMultipartRoute(1024, 1024, quotaTracker);
  const boundary = "----codex-import-multipart-boundary";
  const req = new PassThrough() as PassThrough & {
  ...
  const res = {
    ...
    status() {
      return {
        json() {
          throw new Error("The multipart abort path should not send a response.");
        },
      };
    },
  };
