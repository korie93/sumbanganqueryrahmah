...
export function registerAuthRoutes(app: Express, deps: AuthRouteDeps) {
  ...
  const handleLogin = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  app.get("/api/me", authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
  ...
  app.patch("/api/me/credentials", authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
      return sendCredentialError(res, 401, "PERMISSION_DENIED", "Authentication required.");
    }
    ...
    let updatedUser;
    try {
      updatedUser = await storage.updateUserCredentials({
        userId: actor.id,
  ...
  app.get("/api/admin/users", authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  app.patch("/api/admin/users/:id/credentials", authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user || req.user.role !== "superuser") {
      return sendCredentialError(res, 403, "PERMISSION_DENIED", "Only superuser can access this resource.");
    }
  ...
  app.get("/api/accounts", authenticateToken, requireRole("superuser"), asyncHandler(async (_req: AuthenticatedRequest, res: Response) => {
    const accounts = await storage.getAccounts();
    return res.json(accounts);
  }));

  app.post("/api/users", authenticateToken, requireRole("superuser"), asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    if (!req.user) {
