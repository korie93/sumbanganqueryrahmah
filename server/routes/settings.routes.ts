...
export function registerSettingsRoutes(app: Express, deps: SettingsRouteDeps) {
  ...
  app.get("/api/app-config", authenticateToken, async (_req, res) => {
    try {
      ...
      return res.status(500).json({ message: error?.message || "Failed to load app config" });
    }
  });

  app.get("/api/settings/tab-visibility", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      return res.status(500).json({ message: error?.message || "Failed to load tab visibility" });
    }
  });
