...
function csvEscape(value: unknown, field = ""): string {
  // CSV is the exact-decimal interchange path; never round money through Number.
  const cell = NUMERIC_EXPORT_FIELD.test(field) && /^-?\d+(?:\.\d+)?$/.test(String(value))
    ? String(value) : safeSpreadsheetCell(value, field);
