-- Persist named Billing Principal targets as immutable revisions. Historical
-- source facts are snapshotted so unticking or deleting an import cannot alter
-- a saved target, reconciliation candidate, or calendar result.
...
CREATE TABLE IF NOT EXISTS public.collection_osp_saved_targets (
  ...
  CONSTRAINT collection_osp_saved_targets_created_by_fkey
    FOREIGN KEY (created_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT collection_osp_saved_targets_updated_by_fkey
    FOREIGN KEY (updated_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT collection_osp_saved_targets_deleted_by_fkey
...
CREATE TABLE IF NOT EXISTS public.collection_osp_target_revisions (
  ...
  target_id uuid NOT NULL
    REFERENCES public.collection_osp_saved_targets(id) ON DELETE RESTRICT ON UPDATE CASCADE,
  ...
  created_by text NOT NULL,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT collection_osp_target_revisions_created_by_fkey
    FOREIGN KEY (created_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.collection_osp_target_sources (
  target_revision_id uuid NOT NULL
    REFERENCES public.collection_osp_target_revisions(id) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.collection_osp_client_results (
  ...
  updated_by text NOT NULL,
  updated_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT collection_osp_client_results_created_by_fkey
    FOREIGN KEY (created_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT collection_osp_client_results_updated_by_fkey
    FOREIGN KEY (updated_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS public.collection_osp_manual_reconciliations (
  ...
  void_reason text,
  CONSTRAINT collection_osp_manual_reconciliations_created_by_fkey
    FOREIGN KEY (created_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT collection_osp_manual_reconciliations_updated_by_fkey
    FOREIGN KEY (updated_by)
    REFERENCES public.users(username) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT collection_osp_manual_reconciliations_voided_by_fkey
...
CREATE TABLE IF NOT EXISTS public.collection_osp_manual_reconciliation_audit (
  ...
  request_id text,
  created_at timestamp with time zone NOT NULL DEFAULT now(),
  CONSTRAINT collection_osp_manual_recon_audit_actor_username_fkey
...
--> statement-breakpoint
-- This audit ledger is intentionally insert-only. Application-level controls are
-- insufficient because direct SQL could otherwise rewrite or erase history.
...
EXECUTE FUNCTION public.reject_collection_osp_manual_reconciliation_audit_mutation();
--> statement-breakpoint
-- Runtime bootstrap can precede the users bootstrap on a blank database. Restore
-- actor foreign keys here as well when the tables already existed before this
-- reviewed migration was applied.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
    WHERE conname = 'collection_osp_saved_targets_created_by_fkey'
      AND conrelid = 'public.collection_osp_saved_targets'::regclass
      AND contype = 'f'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'collection_osp_target_revisions_created_by_fkey'
      AND conrelid = 'public.collection_osp_target_revisions'::regclass
      AND contype = 'f'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'collection_osp_client_results_created_by_fkey'
      AND conrelid = 'public.collection_osp_client_results'::regclass
      AND contype = 'f'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'collection_osp_manual_reconciliations_created_by_fkey'
      AND conrelid = 'public.collection_osp_manual_reconciliations'::regclass
      AND contype = 'f'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE conname = 'collection_osp_manual_recon_audit_actor_username_fkey'
      AND conrelid = 'public.collection_osp_manual_reconciliation_audit'::regclass
      AND contype = 'f'
  ) THEN
...
END $$;
--> statement-breakpoint
COMMENT ON TABLE public.collection_osp_target_source_rows IS
'Immutable encrypted/hashed row facts captured for one Billing Principal target revision.';
--> statement-breakpoint
COMMENT ON TABLE public.collection_osp_manual_reconciliation_audit IS
'Append-only superuser audit history for Billing Principal manual reconciliations.';
