...
Current status (2026-09-10): production access is verified. Privacy-safe Nginx diagnostics and bounded API/auth/WS/telemetry edge changes are deployed, backed up and syntax-validated; reload and local/public readiness passed. The active application remains `6cbada45`. Search and WebSocket corrections passed local typecheck, lint, build and focused regressions but still require their own approved release deployment. [Continuation handoff section 19](../CODEX_CONTINUATION_HANDOFF_SQR_429_NAT_AWARE_RATE_LIMIT_FIX.md) is authoritative for exact current evidence, backup paths and remaining gates. Sections 1–21 retain the original implementation history unless explicitly updated; old snapshot limits and original test counts are not current deployment assertions.
...
The extended workflow audit reproduced a separate Search IP collision: 30 authenticated users yielded 10 successes and 20 route-level 429s. A minimal server-authenticated-user key fix retains 10 requests/10 seconds, anonymous IP protection and shared Redis fail-closed behavior. That additional application fix still requires publication/verification/deployment. Explicit 30-user middleware/HTTP tests passed; the extended real Redis case awaits CI.
