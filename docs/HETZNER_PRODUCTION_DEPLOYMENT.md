...
Kekalkan `SQR_MAX_WORKERS=1` untuk production buat masa ini. Redis boleh berkongsi rate-limit counters. Redis pub/sub untuk WebSocket boleh diaktifkan dengan `SQR_WS_SHARED_BUS=redis` untuk mesej broadcast serta cross-worker activity close. Namun 2FA replay cache dan adaptive runtime protection masih process-local; runtime production akan fail fast jika multi-worker diaktifkan sebelum semua runtime security state mempunyai shared implementation dan ujian.
