...
Kekalkan `SQR_MAX_WORKERS=1` untuk production kecil. Jika mahu multi-worker, aktifkan `SQR_RATE_LIMIT_STORE=redis` dengan `SQR_REDIS_RATE_LIMIT_URL` supaya fixed-window rate-limit counters, adaptive runtime protection buckets, dan 2FA replay protection dikongsi merentas worker. Aktifkan juga `SQR_WS_SHARED_BUS=redis` untuk Redis pub/sub untuk WebSocket, termasuk mesej broadcast serta cross-worker activity close. Runtime production akan fail fast jika multi-worker diaktifkan tanpa shared rate/replay store atau WebSocket shared bus.
