...
- `502 Bad Gateway` selepas deploy, build, atau `pm2 restart`: semak `pm2 logs sqr --lines 100` dahulu. Jika log menunjukkan package hilang seperti `ERR_MODULE_NOT_FOUND`, jalankan install deterministic selepas checkout disahkan. Flow deploy ringkas:
  ...
  Jangan teruskan `npm ci`, migration, build, atau PM2 restart jika checkout gate gagal kerana branch salah, working tree kotor, atau commit server belum sama dengan `origin/$BRANCH`.
