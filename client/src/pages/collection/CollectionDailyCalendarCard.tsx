              ...
              {overview.days.map((day) => {
                ...
                return (
                  <button
                    key={day.date}
                    type="button"
                    className={`rounded-md border p-2 text-left text-xs transition-colors hover:brightness-95 ${
                      selectedDate === day.date ? "ring-2 ring-ring ring-offset-1" : ""
                    } ${statusCardClass(day.status)}`}
                    aria-pressed={selectedDate === day.date}
                    aria-label={`${formatDateDDMMYYYY(day.date)} - ${statusLabel(day.status)} - Collected ${formatAmountRM(day.amount)} - Target ${formatAmountRM(day.target)}`}
