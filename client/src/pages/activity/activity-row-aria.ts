...
export function buildActivityRowAriaLabel(activity: ActivityRecord, browserLabel: string) {
  ...
  const device = getActivityDeviceLabel(activity)

  return `Activity for ${activity.username}, role ${activity.role}, status ${activity.status.toLowerCase()}, login ${loginLabel}, logout ${logoutLabel}, device ${device}, browser ${browser}, IP ${ipAddress}`
}
