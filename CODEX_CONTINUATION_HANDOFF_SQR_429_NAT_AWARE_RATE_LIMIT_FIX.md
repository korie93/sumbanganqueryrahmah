...
Latest status (2026-09-10 15:02 UTC): Nginx admission/diagnostics corrections and approved Search/WebSocket application `62cbe7aa20390ddd87ece97c41b1424b8c9154a4` are deployed. CI/live Redis, CodeQL, dispatched Release Verification attempt 2 and production approval passed. Checksums, local/public readiness/full SHA, Redis PONG and post-restart checks passed. Section 21 supersedes earlier deployment/access/publication blockers. Actual 30-person office acceptance remains outstanding; no staff window or real traffic has been provided. Disk is99%used with about1.70GBfree, requiring operator attention before further deployments. Full report: [SQR 429 report](docs/SQR_429_NAT_AWARE_RATE_LIMIT_REPORT.md). Deployment instructions: [active Nginx rollout](docs/SQR_429_ACTIVE_NGINX_ROLLOUT.md).
